...
bool export_c_file( PNG2AssetData* assetData) {
    ...
    fprintf(file, "; Conversion args:%s\n\n", assetData->args->args_for_logging_to_output.c_str());

    fprintf(file, ".module %s\n\n", assetData->args->data_name.c_str());

    // Export Globals
// TODO: if .. metasprite (instead of map)

    if (assetData->args->includedMapOrMetaspriteData) && (assetData->args->export_as_map)
        fprintf(file, ".globl %s_map\n",      assetData->args->data_name.c_str());

    if (assetData->args->includeTileData)
        fprintf(file, ".globl %s_tiles\n",    assetData->args->data_name.c_str());

    if (exportOpt.has_palette_data_to_export)
        fprintf(file, ".globl %s_palettes\n", assetData->args->data_name.c_str());

    // Bankref globals
    fprintf(file, ".globl b___func_%s\n", assetData->args->data_name.c_str());
    fprintf(file, ".globl ___func_%s\n",  assetData->args->data_name.c_str());

    // Start of CODE / constseg data
    if (assetData->args->bank != BANK_NUM_UNSET) {
        if (assetData->args->area_specified)
            fprintf(file, ".area _%s_%d\n\n", assetData->args->area_name.c_str(), assetData->args->bank);
    
        else
            fprintf(file, ".area _CODE_%d\n\n", assetData->args->bank);
    } else
        fprintf(file, ".area _CODE\n\n");

    fprintf(file, "; Bankref"
                  "  b___func_%s = %d\n"
                  "___func_%s::\n"
                  "  .local b___func_%s\n"
                  "  ___bank_%s = b___func_%s\n"
                  "  .globl ___bank_%s\n", assetData->args->data_name.c_str(), assetData->args->bank, 
                  assetData->args->data_name.c_str(),
                  assetData->args->data_name.c_str(),
